...
def call_gemini_api(api_key, system_instruction, user_prompt):
    """
    Calls Google Gemini REST API (gemini-1.5-flash / gemini-2.0-flash) with graceful fallback.
...
class CodeCraftHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_ai(self):
        try:
            ...
            api_key = payload.get("apiKey", "").strip() or os.environ.get("GEMINI_API_KEY", "").strip()
    ...
    def ai_gemini_process(self, api_key, action, language, code, error, prompt):
        ...
        return {
            ...
            "source": "gemini",
            "type": action,
            "title": f"Gemini 1.5 AI — {action.title()}",
            "response": text_out,
